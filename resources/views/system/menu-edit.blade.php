    ...
    <div class="row">
        <div class="col-md-12">
            <div class="box box-info">
                ...
                <form id="post-form" class="form-horizontal" action="{{ route('admin::menu.update', [$menu->id]) }}" method="post" enctype="multipart/form-data" pjax-container>
                    {{ csrf_field() }}
                    {{ method_field('PUT') }}
                    <div class="box-body">
                                ...
                                <div class="col-sm-8">
                                    <select style="width: 100%;" name="parent_id" id="parent_id" tabindex="-1" data-placeholder="选择父级菜单" class="form-control parent_id select2-hidden-accessible">
                                        @foreach($options as $key => $option)
                                            <option value="{{ $key }}" {{ $menu->parent_id == $key ? 'selected' : '' }}>{!! $option !!}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="form-group">
                                ...
                                <div class="col-sm-8">
                                    <div class="input-group">
                                        ...
                                        <input type="text" id="title" name="title" value="{{ $menu->title }}" class="form-control title" placeholder="输入 标题">
                                    </div>
                                </div>
                            </div>
                            <div class="form-group  ">
                                ...
                                <div class="col-sm-8">
                                    <div class="input-group">
                                        <span class="input-group-addon"><i class="fa fa-pencil"></i></span>
                                        <input style="width: 140px" type="text" id="icon" name="icon" value="{{ $menu->icon }}" class="form-control icon" placeholder="输入 图标" />
                                    </div>
                                </div>
                            </div>
                            <div class="form-group">
