...
class Pjax
{
     ...
     * @return string
     */
    protected function fetchJs(Crawler $crawler)
    {
        $content = $crawler->filter('body > script')->each(function (Crawler $node, $i) {
            if ($node->text() != '') {
                return <<<EOF
<script>
{$this->decodeUtf8HtmlEntities($node->html())}
</script>
EOF;
            }
